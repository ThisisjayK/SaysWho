...
|---|---|---|---|
| 1 | The full pipeline, no confidence score, hard denominator check | **Done** | `sayswho/`, 774 tests. G0 to G4 each fail on their target bug. `rates.standing_denominator` raises on a contaminated denominator; `gates.assert_no_confidence_number` runs over every payload before it is returned and before it is written to disk |
