...
Last updated 2026-08-16, day 10 of ten, where day 5 was 2026-08-11. 839 tests, and `tests/test_documents.py`
...
| ~~Judge-human agreement~~ | **Computed 2026-08-16 over 35 blind comparable pairs: kappa 0.304, 95% CI 0.004 to 0.604.** The lower bound does not exclude chance, and it is reported as a wide-interval estimate rather than as a calibration. Per class: `NOT_FOUND_IN_SOURCE` precision and recall both 77.3% (n=22), `SUPPORTED` precision 57.1% (n=7) and recall 44.4% (n=9), `PARTIALLY_SUPPORTED` precision 16.7% (n=6) and recall 25.0% (n=4). That last figure measures a row `TODO.md` has carried since day 6 on the strength of two documents and eight calls, and it points the same way. **Whether `extract.py` explains any of the 13 disagreements is unmeasured, and the claim that it does not is withdrawn as of 2026-08-16.** `goldset.attribution` reported 0 attributed to the extractor, and that is 0 checked rather than 0 found: `extraction_missed` is non-null on 1 of the 45 labels and that pair is an agreement, so the comparison ran on none of the 13. 8 of the 13 carry a pasted passage and 13 of those 14 passages were labelled against pages absent from the fetch cache, which is what leaves the field `None`. The 13 in the withdrawn sentence was `labels_with_a_passage`, which counts agreements too. `FINDINGS.md` item 24. Fixed on day 10: `attribution` reports `attributable_to_extraction` over `disagreements_checked` rather than over `disagreements`, and prints no attribution figure at all when nothing was checked, so this run now reads "not run" instead of "0 of 13". The kappa and the per-class figures are computed without any of this and are unaffected. What this replaces: six human labels of which four were on sources the judge was never asked about, so two were comparable and kappa came out 0.0 over them, which at that n is an arithmetic result rather than a measurement and was reported as one |
...
| Claude `.font-claude-response` verified | **not done, and the evidence to do it does not exist** | Attempted 2026-08-16 with the method that verified ChatGPT and stopped there. That method needs a capture and the stored page behind it, and `captures/` holds **no Claude capture at all**. Nothing to check the chat path against and nothing to re-extract from, so this is not a scheduling gap: it needs a real answer captured on a real page before any of it can start. `.bg-surface-3 .standard-markdown`, the Research artifact selector, stays the only verified one and was verified on 2026-08-11 against one live answer |
| Perplexity selectors verified | **not done, and the evidence was deleted** | 29 Perplexity captures exist and all 29 fired `.prose`. All 29 also name a `page_file` that is **no longer on disk**, so the two checks that carry a verification, the whole-page anchor probe and re-extraction from stored bytes, cannot be run over any of them. `pages/` holds the ten ChatGPT pages and nothing else. `verifiedSelectors` stays empty and `tests/test_extension_manifest.py` still asserts it. Re-verifying needs re-capturing, which re-asks the question and returns a different answer, so it is a new capture session rather than a re-read |
...
| The narration lands on the picture it describes | **fixed, and checked line by line** | It did not. The opening browser shot ran four seconds long and carried the capture click with it, so the panel printing `INCOMPLETE` was legible from 0:12 while the voice introduced capture at 0:49 and treated `INCOMPLETE` as news at 1:03. The click moved to the head of the main browser run and every clip was re-placed against beats measured off the picture rather than estimated |
| The answer level refusal happens where its box is | **fixed by rewriting two lines** | It was spoken at 3:55, about forty five seconds after the "No overall score" box had scrolled away, and it could not simply move, because it opened "then the second refusal" and so depended on the grey claims at 3:44 coming first. The refusal moved into the line that sits over that box and now leads it, since the box is readable from 3:02.8 to about 3:11 and never returns. The slot it vacated got a line about what is on screen there, the sources and the gate G1 skip, checked against `reports/report-chatgpt-20260816T2304300000.json`: two sources, one `SOURCE_OK` and one `SOURCE_DEAD_LINK`, and 8 skipped lines. **The cost, taken deliberately:** both refusals are still spoken but the explicit "first, then second" framing is gone. They now arrive in the order the report itself presents them, and the claim level one still ends on "a number here is invented" |
