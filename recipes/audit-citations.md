...
|---|---|---|---|
| E | Ethics | Privacy and honesty both pass | The run stops before fetching. `.venv/bin/python tools/ethics_gate.py` names which check failed and what to do. `tools/run_stratum.py` runs it first and refuses to continue |
...
| `.venv/bin/python tools/ethics_gate.py` | The privacy and honesty gate. Exits non-zero when either half fails, and `run_stratum.py` runs it before the honest run |
...
6. **The ethics gate.** `.venv/bin/python tools/ethics_gate.py` checks privacy against git rather than against the
