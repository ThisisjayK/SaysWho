...
NOT_A_CLAIM_ABOUT_THE_REPO = {
    "CLAUDE.md": "instructions to a tool, not a published claim about the repo",
    # Generated by the runner from one run's own record, not written. A prose gate over them would be
    # checking the harness's output formatting against the repo, which is what test_harness.py does.
    "runs/day9/RUN_LOG.md": "a run artefact, generated from the run record rather than authored",
    "runs/day9/TRACE.md": "a run artefact, generated from the run record rather than authored",
}
