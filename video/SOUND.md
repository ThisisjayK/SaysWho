...
**This file covers the Remotion short only.** The live demo has its own fifteen lines, written in
`scripts/explainer.py` next to the offset each is measured against, and regenerated by
`scripts/explainer_tts.py`. Same voice, so the two halves of `../site/media/sayswho.mp4` match. The rules
below hold on both sides: no music, silence is the texture rather than an absence, and nothing sounds
triumphant.
